...
namespace CompareWindows.View.Window {
    public partial class SvnWindow : Form {
        ...
        private void inputMenuItem_Click(object sender, EventArgs e) {
            ...
            SvnInputWindow win = new SvnInputWindow();
            AddOwnedForm(win);
            win.OnConfirm += OnSvnInputConfirm;
            win.ShowDialog();
        }

        private void OnSvnInputConfirm(IEnumerable<string> list) {
            if (string.IsNullOrEmpty(leftRoot) || string.IsNullOrEmpty(rightRoot)) {
                MessageBox.Show("请先选择对比的文件夹");
                return;
            } // end if
